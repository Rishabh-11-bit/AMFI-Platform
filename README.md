...
```bash
pip install -r amfi_v4_complete/requirements.txt
```

### 2. Set up database
```bash
python amfi_v4_complete/scripts/setup_db.py
```

### 3. Configure environment
```bash
cp amfi_v4_complete/.env.example amfi_v4_complete/.env
...
```bash
# Linux:
bash amfi_v4_complete/scripts/setup_ollama.sh
...
### 5. Run
```bash
python amfi_v4_complete/run.py
...
```bash
cp amfi_v4_complete/.env.example amfi_v4_complete/.env
bash amfi_v4_complete/docker-start.sh
# Dashboard: http://localhost:8000
```
...
```
amfi_v4_complete/     ← backend + agent core
  backend/
    agent/            ← classifier, executor, LLM, procedures, tools
    routers/          ← REST API, auth, webhooks, websocket
    services/         ← NMS connectors, ITSM, monitoring, notifications
  frontend/           ← React dashboard (Vite)
  scripts/            ← setup, seed, export, cleanup utilities
  run.py              ← entry point
  docker-compose.yml
amfi_v4_frontend/     ← standalone frontend build
```
...
```bash
python amfi_v4_complete/scripts/export_training.py
```
